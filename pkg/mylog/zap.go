...
func InitLogger() {
	log.Println(color.CyanString("初始化ZAP日志配置...."))
	absPath, err := filepath.Abs(config.Cfg.LogCfg.File)
	if err != nil {
		panic(err)
	}
<<<<<<< HEAD
...
=======
	zapLogger, err := logger.NewJSONLogger(logger.WithTimeLayout("2006-01-02 15:04:05"), logger.WithFileRotationP(absPath))
>>>>>>> 137f07b (新增 1.添加viper配置框架以及zap日志框架 修改 1.将原setting包改为pkg包,并将不同功能的模块函数各自独立为一个单独包)
	if err != nil {
		panic(err)
	}
...
func NewJSONLogger(opts ...Option) (*zap.Logger, error) {
	level := Level(config.Cfg.LogCfg.Level)
	opt := &option{level: level, fields: make(map[string]string), disableConsole: false}
	for _, f := range opts {
		f(opt)
	}
	...
	encoderConfig2 := zapcore.EncoderConfig{
		TimeKey:       "time",
		LevelKey:      "level",
		NameKey:       "logger", // used by logger.Named(key); optional; useless
		CallerKey:     "caller",
	...
	consoleEncoder := zapcore.NewConsoleEncoder(encoderConfig2)
