...
<<<<<<< HEAD:setting/system.go
<<<<<<< Updated upstream:setting/system.go
	Logger.Debugf("setRTC %v", rtc)
=======
>>>>>>> Stashed changes:pkg/system/system.go
=======
	mylog.Logger.Debugf("setRTC %v", rtc)
>>>>>>> 137f07b (新增 1.添加viper配置框架以及zap日志框架 修改 1.将原setting包改为pkg包,并将不同功能的模块函数各自独立为一个单独包):pkg/system/system.go
